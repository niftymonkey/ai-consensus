...
export default function SignInPage() {
  ...
  const handlePreviewSignIn = async (e: React.FormEvent) => {
    ...
    if (result?.error) {
      setError("Email not in allowlist. Contact the project owner for access.");
      setIsLoading(false);
    } else if (result?.ok) {
  ...
  // Preview deployment: show email-only login
  if (isPreviewDeployment) {
    return (
      <div className="container flex items-center justify-center min-h-[calc(100vh-8rem)] py-12">
        <Card className="w-full max-w-md">
          <CardHeader className="space-y-1 text-center">
            <CardTitle className="text-2xl font-bold">Preview Sign In</CardTitle>
            <CardDescription>
              This is a preview deployment. Enter your email to sign in.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <p className="text-xs text-center text-muted-foreground pt-4">
              Only allowlisted emails can access preview deployments.
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  // Production: show OAuth providers
  return (
    <div className="container flex items-center justify-center min-h-[calc(100vh-8rem)] py-12">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>
            Choose a provider to sign in to AI Consensus
          </CardDescription>
        </CardHeader>
